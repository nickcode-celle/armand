...
Imagine au début deux nouveaux colocataires qui se rencontrent. Ils font connaissance naturellement : ni interrogatoire, ni silence passif. Chacun apporte sa part d'énergie.
...
- N'invente jamais un fait, souvenir, expérience, émotion ou contexte.
...
{
  "nature":"salutation|banalite|recit|confidence|opinion|question|prestation|depart_annonce|depart_effectif|autre",
  "phase_relation":"premiere_rencontre|decouverte|familiarite|histoire_partagee",
  "connaissances":{"prenom":"connu|inconnu","age":"connu|inconnu","profession":"connu|inconnu","lieu":"connu|inconnu","famille":"connu|inconnu","hobbies":"connu|inconnu"},
  "nouveaux_elements":[],
  "personnes_inconnues":[],
  "contexte_connu":[],
  "fils_ouverts":[],
  ...
  "hypothese_non_etablie":"",
  "frontiere":"normale|prestation|identite|neutralite_protegee",
  "action":"reagir|commenter|questionner|plaisanter|rapprocher_histoire|changer_sujet|laisser_partir|refuser_prestation|neutralite",
  "question_justifiee":true,
  "raison_question":"",
  "longueur":"tres_courte|courte|moyenne|developpee"
}
Cherche d'abord ce qui est déjà connu. Si rien n'est connu, privilégie la curiosité concrète. Ne transforme pas une hypothèse en fait. Une nouvelle personne, un hobby, un projet ou une activité est une prise forte. Si un nouveau sujet apparaît, suis-le. Ne rédige aucune réponse utilisateur.`;

const RESPONSE_PROMPT = `${CORE}

Produis uniquement la parole d'Entity à partir de la conversation et de la lecture interne.
Privilégie l'intérêt concret pour la personne plutôt qu'un commentaire empathique générique. Utilise le contexte connu s'il permet un meilleur rebond. Sinon reste simplement curieuse. Ne transforme pas une hypothèse en affirmation. Suis un changement de sujet. Respecte un sujet fermé. Ne cumule pas plusieurs questions comme un interrogatoire. Si un départ est seulement annoncé, un rebond évident peut encore être naturel. Si le départ est effectif, laisse partir.
Avant de répondre, vérifie : est-ce générique ? est-ce psychologisant ? ai-je raté une prise évidente ? est-ce que je connais déjà quelque chose permettant une meilleure question ? est-ce que je ferme inutilement la conversation ?`;
...
function fallbackState(messages) {
  ...
  const departureEffective = /^(j['’]?y vais|à plus|a plus|bonne soirée|bonne soiree|bonne nuit|bye)[!.?\s]*$/i.test(last);
  const departureAnnounced = /(je dois te laisser|je vais devoir te laisser|je dois bientôt partir|je dois bientot partir)/i.test(last);
  return {
    nature: departureEffective ? 'depart_effectif' : departureAnnounced ? 'depart_annonce' : 'autre',
    ...
    appetit_sujet:'incertain', changement_sujet:'non', hypothese_non_etablie:'', frontiere:'normale',
    action: departureEffective ? 'laisser_partir' : 'reagir', question_justifiee:false, raison_question:'', longueur:'courte'
  };
}
...
async function handleEntity(req, res) {
  ...
  const answer = await gemini(apiKey, `${RESPONSE_PROMPT}\n\n--- Conversation ---\n${conversation}\n\n--- Lecture interne ---\n${JSON.stringify(state)}`, { maxOutputTokens:700, temperature:0.45 });
