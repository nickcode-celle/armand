...
export function createPostReplyEvolution({storage}){
  return async function evolveAfterObservation({entityId,observerRecord}){
    ...
    return storage.mutate(id,'evolution-state',{evolution:null,last_observer_revision:null,last_observer_at:null},current=>{
      ...
      const evolution={...initialized,durable_levels:applied.levels,history_level:historyLevel,history_events:historyEvents.slice(-200),history_last_change:historyChange,observer_last:observerRecord.output,last_changes:applied.changes,updated_at:observerAt};
