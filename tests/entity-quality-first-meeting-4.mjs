...
const entityId=`quality-first-${Date.now()}-${Math.random().toString(36).slice(2)}`;
const turns=[
  "Salut.",
...
let messages=[];
console.log('ENTITY PREMIÈRE RENCONTRE — 4 appels\n');
for(let i=0;i<turns.length;i++){
  messages.push({role:'user',content:turns[i]});
  const r=await fetch(API,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({entityId,requestId:`first-${i}-${crypto.randomUUID()}`,messages})});
