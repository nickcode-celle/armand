...
const root=fs.mkdtempSync(path.join(os.tmpdir(),'entity-behavior-'));try{
 ...
 const other={faits:[{id:'secret',sujet:'Autre',propriete:'ville',valeur:'Bordeaux',importance:1,structurel:true}]};await orchestrate('other-entity',other,'Bordeaux',ai,state);const isolated=await storage.get('camille','embedding-cache',{});assert.ok(!Object.values(isolated).some(x=>JSON.stringify(x).includes('Bordeaux')),'Entity embedding storage must be isolated by identity');
