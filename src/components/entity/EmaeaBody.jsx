...
export default function EmaeaBody({entityId}){
  ...
  return <div className="relative mx-auto w-full">
    <style>{`
      ...
      /* Bille gratuite : rendu or métallique, avec lumière, reflet et profondeur. */
      body:has([aria-label="EMÆÄ"]) main .mb-4.grid.grid-cols-2.gap-2 > :nth-child(2) > span:first-child{
        background:
          radial-gradient(circle at 31% 25%,rgba(255,255,255,.98) 0 5%,rgba(255,244,176,.95) 7%,transparent 18%),
          radial-gradient(circle at 36% 31%,#fff1a8 0%,#f6c84b 24%,#c98a16 54%,#6f4307 82%,#2f1a02 100%)!important;
        border-color:rgba(255,224,120,.88)!important;
        box-shadow:0 0 12px rgba(234,179,8,.42),inset -3px -4px 7px rgba(63,36,0,.52),inset 3px 3px 5px rgba(255,248,202,.52)!important;
      }
      ...
      /* Deux derniers onglets : billes de 24 px, comme les deux premières. */
      ...
      body:has([aria-label="EMÆÄ"]) main .mb-4.grid.grid-cols-2.gap-2::before{
        content:"      0";
        background:
          radial-gradient(circle 12px at 20px 50%,rgba(255,255,255,.95) 0 5%,#93c5fd 10%,#3b82f6 42%,#1d4ed8 72%,#172554 100%) no-repeat,
          linear-gradient(180deg,rgba(6,78,59,.66),rgba(4,47,46,.60));
      ...
      body:has([aria-label="EMÆÄ"]) main .mb-4.grid.grid-cols-2.gap-2::after{
        ...
        background:
          radial-gradient(circle 12px at 20px 50%,rgba(255,255,255,.98) 0 5%,#fff1a8 9%,#f6c84b 32%,#c98a16 61%,#6f4307 84%,#2f1a02 100%) no-repeat,
          linear-gradient(180deg,rgba(113,63,18,.68),rgba(66,42,10,.64));
        background-size:24px 24px,100% 100%;
        background-position:8px center,0 0;
